defmodule Relay.OpenTelemetry do
  @moduledoc """
  ...
  can define custom spans using the `Relay.OpenTelemetry.trace/1` macro:
  ...
  ## Tracing All Functions & Macros

  As a convenience, if you would like to trace all public functions and macros defined in a module, you can use the
  `trace_all/1` macro:
  ...
  You can pass the following options to `use Relay.OpenTelemetry` to configure the calling module:

  * `debug` - should debug statements be printed to stdout during compilation? Defaults to `false`.
  """

  alias Relay.OpenTelemetry.Encoder
  ...
  defmacro __using__(opts \\ []) do
    ...
    quote do
      ...
      Module.put_attribute(__MODULE__, :on_definition, Relay.OpenTelemetry)
      Module.put_attribute(__MODULE__, :before_compile, Relay.OpenTelemetry)
          ...
          unquote(kind)(unquote(name)(unquote_splicing(positional_args))) do
            OpenTelemetry.Tracer.with_span unquote(span_name),
              ...
              try do
                ...
                Relay.OpenTelemetry.__record_arguments__(
                  unquote(positional_args),
                  unquote(record_arguments)
                )

                result = super(unquote_splicing(positional_args))
                Relay.OpenTelemetry.__record_results__(result, unquote(record_results))
                result
              rescue
                exception ->
                  Relay.OpenTelemetry.__record_errors__(exception, unquote(record_errors))
  ...
  * `thread.name` - the `PID` of the process executing the annotated function.
  ...
  Following Elixir's naming conventions, functions and macros prefixed with an underscore are
  ignored by the `trace/1` macro.
  ...
  defmacro trace(opts \\ []) when is_list(opts) do
    quote bind_quoted: [opts: opts] do
      Relay.OpenTelemetry.__trace__!(__MODULE__, __ENV__.line, __ENV__.file, opts)
    end
  end

  @doc """
  Annotates all public functions and macros defined in a module to generate OpenTelemetry traces
  upon invocation.
  ...
  defmacro trace_all(opts \\ []) when is_list(opts) do
    quote bind_quoted: [opts: opts] do
      Relay.OpenTelemetry.__trace_all__!(__MODULE__, __ENV__.line, __ENV__.file, opts)
    end
  end
  ...
  defp register_function_trace!(%{module: module, line: line, file: file}, kind, name, args) do
    ...
    if trace && not underscore_fun?(name) do
  ...
  # hidden functions prefixed by a `_` or surrounded by `__` are never traced
  # https://hexdocs.pm/iex/IEx.html#module-autocomplete
  # https://hexdocs.pm/elixir/naming-conventions.html#underscore-_foo
  defp underscore_fun?(name) do
    stringified = Atom.to_string(name)
    String.starts_with?(stringified, "_")
  end
