defprotocol Relay.OpenTelemetry.Encoder do
  @moduledoc """
  ...
  must ensure the `Relay.OpenTelemetry.Encoder.encode/1` function returns a value with a type the
...
defimpl Relay.OpenTelemetry.Encoder, for: Any do
  def encode(_term) do
    :otel_no_encoder
  end
end

defimpl Relay.OpenTelemetry.Encoder, for: [PID, Port, Reference, Function] do
  def encode(term) do
    inspect(term)
  end
end

defimpl Relay.OpenTelemetry.Encoder, for: [Integer, Float, BitString, Atom] do
  def encode(term) do
    term
  end
end

defimpl Relay.OpenTelemetry.Encoder, for: Map do
  def encode(term) do
    Enum.into(term, Map.new(), fn {key, value} ->
      {key, Relay.OpenTelemetry.Encoder.encode(value)}
    end)
  end
end

defimpl Relay.OpenTelemetry.Encoder, for: Tuple do
  def encode(term) do
    term
    |> Tuple.to_list()
    |> Relay.OpenTelemetry.Encoder.encode()
  end
end

defimpl Relay.OpenTelemetry.Encoder, for: List do
  def encode(term) do
  ...
  defp encode_proper_list(term) do
    encoded = for item <- term, do: Relay.OpenTelemetry.Encoder.encode(item)
