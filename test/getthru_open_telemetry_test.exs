defmodule GetthruOpenTelemetryTest do
  use ExUnit.Case
  ...
  describe "__using__/1" do
    test "logs expanded macros to stdout when debug is enabled" do
      stdout =
        ExUnit.CaptureIO.capture_io(fn ->
          defmodule Debugging do
            use Relay.OpenTelemetry, debug: true
      ...
      assert stdout == ~S'''
             def fun(arg0) do
               OpenTelemetry.Tracer.with_span "Relay.OpenTelemetryTest.Debugging.fun/1",
                 kind: :internal,
                 attributes: %{
                   "code.filepath" => "./test/relay/open_telemetry_test.exs",
                   "code.function" => "fun/1",
                   "code.lineno" => 24,
                   "code.namespace" => "Relay.OpenTelemetryTest.Debugging"
                 } do
                 try do
                   Relay.OpenTelemetry.__record_threads__()
                   Relay.OpenTelemetry.__record_arguments__([arg0], false)
                   result = super(arg0)
                   Relay.OpenTelemetry.__record_results__(result, false)
                   result
                 rescue
                   exception ->
                     Relay.OpenTelemetry.__record_errors__(exception, true)
    ...
    test "raises when given an unknown use option" do
      ...
      assert_raise CompileError, msg, fn ->
        defmodule UnknownOpt do
          use Relay.OpenTelemetry, foo: :bar
        end
      end
    end
    ...
    test "warns when there are no calls to trace/1 or trace_all/1" do
      stderr =
        ExUnit.CaptureIO.capture_io(:stderr, fn ->
          defmodule Unused do
            use Relay.OpenTelemetry
          end
        end)

      assert stderr =~ """
             use Relay.OpenTelemetry called but no traces are defined, are you missing trace/1 or trace_all/1?
             """
    end
  end
  ...
  defimpl Relay.OpenTelemetry.Encoder, for: Impl do
    def encode(term) do
  ...
  describe "trace/1" do
    test "emits a span when the corresponding public function is invoked" do
      ...
      assert_receive {:span, span(name: "Relay.OpenTelemetryTest.Traced.my_public_fun/1")}
    end

    test "emits a span when the corresponding private function is invoked" do
      assert Traced.call_private_fun(:foo) == :foo
      assert_receive {:span, span(name: "Relay.OpenTelemetryTest.Traced.my_private_fun/1")}
    end

    test "supports tracing functions with guards" do
      assert Traced.my_fun_with_guards("foo") == "FOO"
      assert_receive {:span, span(name: "Relay.OpenTelemetryTest.Traced.my_fun_with_guards/1")}
    end

    test "supports tracing functions multiple arguments" do
      assert Traced.my_multi_arity_fun(:a, :b, :c) == {:a, :b, :c}
      assert_receive {:span, span(name: "Relay.OpenTelemetryTest.Traced.my_multi_arity_fun/3")}
    end

    test "supports tracing functions with unused arguments" do
      assert Traced.fun_with_unused_args(:a, :b) == :a
      assert_receive {:span, span(name: "Relay.OpenTelemetryTest.Traced.fun_with_unused_args/2")}
    end
    ...
    test "supports tracing functions with unused default arguments" do
      ...
      assert_receive {:span,
                      span(name: "Relay.OpenTelemetryTest.Traced.fun_with_unused_default/2")}
    end
    ...
    test "supports tracing functions with patterns in arguments" do
      ...
      assert_receive {:span,
                      span(name: "Relay.OpenTelemetryTest.Traced.fun_with_pattern_match/1")}
    end
    ...
    test "supports tracing functions with an unused, unnamed argument" do
      ...
      assert_receive {:span,
                      span(name: "Relay.OpenTelemetryTest.Traced.fun_with_unnamed_unused_arg/1")}
    end
    ...
    test "supports tracing functions with multiple heads" do
      ...
      assert_receive {:span,
                      span(name: "Relay.OpenTelemetryTest.Traced.fun_with_multiple_heads/1")}
    ...
    test "supports custom, static span attributes" do
      ...
      assert_receive {:span,
                      span(
                        name: "Relay.OpenTelemetryTest.Traced.fun_with_custom_attrs/0",
    ...
    test "supports setting the :internal span kind" do
      ...
      assert_receive {:span,
                      span(
                        name: "Relay.OpenTelemetryTest.Traced.internal_span_fun/0",
                        kind: :internal
                      )}
    end
    ...
    test "supports setting the :producer span kind" do
      ...
      assert_receive {:span,
                      span(
                        name: "Relay.OpenTelemetryTest.Traced.producer_span_fun/0",
                        kind: :producer
                      )}
    end
    ...
    test "supports setting the :consumer span kind" do
      ...
      assert_receive {:span,
                      span(
                        name: "Relay.OpenTelemetryTest.Traced.consumer_span_fun/0",
                        kind: :consumer
                      )}
    end
    ...
    test "supports setting the :client span kind" do
      ...
      assert_receive {:span,
                      span(
                        name: "Relay.OpenTelemetryTest.Traced.client_span_fun/0",
                        kind: :client
                      )}
    end
    ...
    test "supports setting the :server span kind" do
      ...
      assert_receive {:span,
                      span(
                        name: "Relay.OpenTelemetryTest.Traced.server_span_fun/0",
                        kind: :server
                      )}
    end
    ...
    test "injects code dynamic span attributes" do
      ...
      assert_receive {:span,
                      span(
                        name: "Relay.OpenTelemetryTest.Traced.my_public_fun/1",
                        kind: :internal,
                        attributes:
                          {:attributes, _, _, _,
                           %{
                             "code.function" => "my_public_fun/1",
                             "code.namespace" => "Relay.OpenTelemetryTest.Traced",
                             "code.filepath" => "./test/relay/open_telemetry_test.exs",
                             "code.lineno" => 99
                           }}
                      )}
    ...
    test "links to parent span if one exists in the current ctx" do
      ...
      assert_receive {:span,
                      span(name: "Relay.OpenTelemetryTest.Traced.parent/0", span_id: parent_id)}

      assert_receive {:span,
                      span(
                        name: "Relay.OpenTelemetryTest.Traced.child/0",
    ...
    test "records Integer values" do
      ...
      assert_receive {:span,
                      span(
                        name: "Relay.OpenTelemetryTest.Traced.recorded_argument/1",
                        kind: :internal,
      ...
      assert_receive {:span,
                      span(
                        ...
                        attributes:
                          {:attributes, _, _, _,
                           %{
                             "code.arguments.arg0" => "#Relay.OpenTelemetryTest.Impl<data: :data>"
                           }}
                      )}
    end
    ...
    test "records the result of the function invocation" do
      ...
      assert_receive {:span,
                      span(
                        name: "Relay.OpenTelemetryTest.Traced.recorded_results/1",
                        kind: :internal,
    ...
    test "captures, records, and reraises exceptions" do
      ...
      assert_receive {:span,
                      span(
                        name: "Relay.OpenTelemetryTest.Traced.raises/0",
  ...
  describe "trace_all/1" do
    test "traces all public functions in a module with `trace_all`" do
      ...
      assert_receive {:span,
                      span(
                        name: "Relay.OpenTelemetryTest.TraceAll.fun_one/1",
                        kind: :internal,
                        attributes: {:attributes, _, _, _, %{"foo" => :bar}}
                      )}
      ...
      assert_receive {:span,
                      span(
                        name: "Relay.OpenTelemetryTest.TraceAll.fun_two/1",
                        kind: :internal,
                        attributes: {:attributes, _, _, _, %{"foo" => :bar}}
                      )}
      ...
      assert_receive {:span,
                      span(
                        name: "Relay.OpenTelemetryTest.TraceAll.fun_raises/0",
                        kind: :internal,
                        attributes: {:attributes, _, _, _, %{"foo" => :bar}},
      ...
      assert_receive {:span,
                      span(
                        name: "Relay.OpenTelemetryTest.TraceAll.call_private_fun/0",
                        kind: :internal,
                        attributes: {:attributes, _, _, _, %{"foo" => :bar}}
                      )}
      ...
      refute_receive {:span,
                      span(
                        name: "Relay.OpenTelemetryTest.TraceAll.private_fun/0",
                        kind: :internal,
                        attributes: {:attributes, _, _, _, %{"foo" => :bar}}
                      )}
    end

    test "ignores exported functions intended to be hidden" do
      assert TraceAll.__hidden_fun__() == :ok

      refute_receive {:span,
                      span(
                        name: "Relay.OpenTelemetryTest.TraceAll.__hidden_fun__/0",
                        kind: :internal,
                        attributes: {:attributes, _, _, _, %{"foo" => :bar}}
                      )}

      assert TraceAll._hidden_fun() == :ok

      refute_receive {:span,
                      span(
                        name: "Relay.OpenTelemetryTest.TraceAll._hidden_fun/0",
                        kind: :internal,
                        attributes: {:attributes, _, _, _, %{"foo" => :bar}}
                      )}
    end
  ...
  describe "trace_attrs/1" do
    test "injects dynamic attributes into the current span" do
      ...
      assert_receive {:span,
                      span(
                        name: "Relay.OpenTelemetryTest.TraceAttrs.fun/1",
                        kind: :internal,
    ...
    test "encodes dynamic attributes with Encoder protocol" do
      ...
      assert_receive {:span,
                      span(
                        name: "Relay.OpenTelemetryTest.TraceAttrs.fun2/0",
                        kind: :internal,
                        attributes:
                          {:attributes, _, _, _,
                           %{
                             "foo" => :bar,
                             "dynamic" => "#Relay.OpenTelemetryTest.Impl<data: :data>"
                           }}
                      )}
    end
  end
